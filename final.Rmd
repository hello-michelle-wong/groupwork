...
2. Independence: Looking at the plot above, the residuals from the model appear to be independent. The residuals seem to be randomly distributed and fluctuate evenly above and below zero. Although we should acknowledge that residual plots alone do not prove independence—they provide evidence for it.
...
4. No Perfect Multicollinearity: In regression analysis, it is crucial that the independent variables are not highly correlated with one another, as this can make it challenging to isolate the individual effects of each predictor. While this is typically not an issue when the independent variables are categorical or factors in ANOVA, it becomes more relevant in multiple regression.

Variance Inflation Factor (VIF) is commonly used to assess multicollinearity. VIF < 5 indicates acceptable (low) multicollinearity. VIF between 5 and 10 signals moderate multicollinearity and may warrant further investigation. VIF > 10 is considered problematic (high multicollinearity), and such values should generally be avoided.

In our analysis, the VIF values for all variables are well below 5, which indicates that there is no significant multicollinearity present in the model. This suggests that the predictor variables are independent, and we can confidently interpret their individual effects on the outcome.
...
Since the assumptions hold, we are able to use a linear regression model to examine the relationships between our variables. The regression model we constructed investigates how the area of maximal compression (amc), history of various medical conditions (hist1 through hist5), and their interactions affect the end-tidal CO2 (etCO2) levels. We decided to retain the interaction terms between amc:hist1 (history of coronary artery disease) and amc:hist3 (history of chronic kidney disease) based on clinical reasoning, despite their lack of statistical significance. These interactions were included to explore potential nuanced effects on etCO2 levels that could be clinically meaningful. Although the coefficients for these interactions were not statistically significant, their retention helps ensure a comprehensive understanding of the relationships at play.
